...
import { relaxGateForOpenWebOsint } from './customer-boundaries.js';
...
export async function processOneFulfillmentJob({ ownerEmail, maxAttempts = 5, caseRef: requestedCaseRef = null, deps = {} }) {
  ...
  const launchGate = relaxGateForOpenWebOsint(evaluateLaunchGate(packageId, process.env), packageId);
  ...
  if (!launchGate.launchReady) {
    ...
    await recordAuditEvent({ event: 'launch_gate_blocked_fulfillment_job', caseRef, jobId: job.id, packageId, blockingChecks: launchGate.launchBlockingDetails.map((detail) => detail.id) });
  ...
  const orderInput = existingOrder ? resolveInvestigationInput(existingOrder) : {
    packageId,
  ...
  if (canAssessOrderCoverage) {
    const orderGate = relaxGateForOpenWebOsint(evaluateOrderGate(packageId, orderInput, process.env), packageId);
    if (!orderGate.launchReady) {
      ...
      await recordAuditEvent({ event: 'order_coverage_blocked_fulfillment_job', caseRef, jobId: job.id, packageId, location: orderGate.orderCoverage?.locationLabel || '', blockingChecks: orderGate.launchBlockingDetails.map((detail) => detail.id) });
      await completeJob(job.id);
  ...
  try {
    ...
    await upsertOrder(caseRef, { lastProcessedAt: new Date().toISOString(), lastError: null, retryAt: null });
    await completeJob(job.id);
    ...
    await upsertOrder(caseRef, { status: terminal ? ORDER_STATUS.FAILED : ORDER_STATUS.QUEUED, lastError: message, retryAt });
    if (terminal) await recordDeadLetter({ caseRef, jobId: job.id, error: message, source: 'process-queue' });
