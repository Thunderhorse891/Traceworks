...
test('validateStripeSecretKey accepts restricted and secret server keys and rejects publishable keys', () => {
  assert.equal(validateStripeSecretKey('sk_test_123').ok, true);
  assert.equal(validateStripeSecretKey('sk_live_123').ok, true);
  assert.equal(validateStripeSecretKey('rk_test_123').ok, true);
  assert.equal(validateStripeSecretKey('rk_live_123').ok, true);
