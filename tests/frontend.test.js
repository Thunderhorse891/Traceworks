...
test('client packages include valid Stripe payment links', () => {
  ...
  for (const pkg of clientPackages) {
    assert.ok(pkg.id,   `Package ${pkg.id || '?'} missing id`);
    assert.ok(pkg.name, `Package ${pkg.id} missing name`);
    assert.ok(typeof pkg.price === 'string' && pkg.price.length > 0, `Package ${pkg.id} missing price`);
    assert.ok(Array.isArray(pkg.bullets) && pkg.bullets.length >= 3, `Package ${pkg.id} needs 3+ bullets`);
  }
