...
export const WIX_WIDGET_TAG_NAME = 'traceworks-landing';
export const DEFAULT_WIX_APP_BASE_URL = 'https://traceworks.example.com';

const wixPackages = clientPackages.map((pkg) => ({
  id: pkg.id,
  name: pkg.name,
  price: pkg.price,
  summary: pkg.summary,
  ...
  includedFindings: Array.isArray(pkg.includedFindings) ? pkg.includedFindings.slice(0, 3) : [],
}));

export function buildWixLandingElement({
  appBaseUrl = DEFAULT_WIX_APP_BASE_URL,
  packages = wixPackages,
} = {}) {
  const packageJson = JSON.stringify(packages);
  const defaultBase = JSON.stringify(appBaseUrl);

  return `(() => {
  const WIDGET_TAG = ${JSON.stringify(WIX_WIDGET_TAG_NAME)};
  const DEFAULT_APP_BASE_URL = ${defaultBase};
...
export function buildWixPreviewHtml({
  appBaseUrl = DEFAULT_WIX_APP_BASE_URL,
  scriptFile = './traceworks-landing.element.js',
} = {}) {
  return `<!doctype html>
<html lang="en">
  <head>
    ...
    <title>TraceWorks Wix Preview</title>
    <style>
      body {
        margin: 0;
        min-height: 100vh;
        background:
          radial-gradient(circle at top left, rgba(212,168,39,0.09), transparent 28%),
          linear-gradient(180deg, #07101b 0%, #0a111b 48%, #0d1520 100%);
  ...
  <body>
    <div class="shell">
      <p class="note">This is the generated Wix custom element preview. Replace <code>app-base-url</code> with your real TraceWorks domain before placing it in Wix Studio.</p>
      <traceworks-landing app-base-url="${appBaseUrl}"></traceworks-landing>
    </div>
